...
export const composeMain = (
  ...
  switch (localSettings.composeGenerationMode) {
    ...
    case "composable":
      ...
      return getComposableTemplate(stringToClassName(sceneNode[0].name), result);
    case "screen":
      if (!result.startsWith("Column")) {
        result = generateComposeWidget("Column", { content: [result] });
      }
      return getFullScreenTemplate(stringToClassName(sceneNode[0].name), result);
  }

  return result;
};

const generateComposeWidget = (widget: string, props: Record<string, any>): string => {
...
const composeFrame = (
  ...
  if (node.layoutMode !== "NONE") {
    ...
    if (node.isAsset) {
      return composeContainer(node, "Icon(Icons.Default.Home, contentDescription = null)");
    }
