 ...
 * example: shadow
 */
export const tailwindShadow = (node: BlendMixin & { type: string }): string[] => {
  ...
  // Handle text shadows differently
  if (node.type === "TEXT") {
    return node.effects
      .map((d) => {
        if (d.type === "DROP_SHADOW" && d.visible !== false) {
          ...
          return `[text-shadow:_${offsetX}px_${offsetY}px_${radius}px_rgb(${r}_${g}_${b}_/_${a})]`;
        }
        return "";
      })
      .filter(Boolean);
  ...
  // box-shadow logic for non-text elements
