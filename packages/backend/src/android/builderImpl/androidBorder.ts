...
import { AndroidSolidColor } from "./androidColor";
import { Modifier, androidElement } from "./androidParser";

const androidStroke = (node: SceneNode): number => {
 ...
 */
export const androidBorder = (node: SceneNode): string[] | null => {
  if (!("strokes" in node) || !node.strokes || node.strokes.length === 0) {
    return null;
  }

  const width = androidStroke(node);
  ...
  return node.strokes
    .map((stroke) => {
      const strokeColor = AndroidSolidColor(stroke);
      ...
      if (strokeColor) {
        return new androidElement(getViewType(node))
          .addModifier(inset)
 ...
 * The reason this was extracted into its own method is for reusability in [androidBorder],
 ...
 * @returns a string with RoundedRectangle, if node has a corner larger than zero; else "".
 */
export const androidRoundedRectangle = (node: SceneNode): string => {
  const corner = androidCornerRadius(node);
  if (corner) {
    return `RoundedRectangle(cornerRadius: ${corner})`;
  }
